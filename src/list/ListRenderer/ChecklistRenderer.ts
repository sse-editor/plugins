import { IconCheck } from "@sse-editor/icons";
import type { ChecklistItemMeta } from "../types";
import type { ListConfig } from "../types";
import { isEmpty, make } from "@sse-editor/dom";
...
export class CheckListRenderer
  implements ListRendererInterface<ChecklistItemMeta>
{
  /**
  ...
  public renderWrapper(isRoot: boolean): HTMLUListElement {
    ...
    if (isRoot === true) {
      wrapperElement = make("ul", [
        CheckListRenderer.CSS.wrapper,
        CheckListRenderer.CSS.checklist,
      ]) as HTMLUListElement;
      ...
      wrapperElement.addEventListener("click", (event) => {
        ...
        if (target) {
          const checkbox = target.closest(
            `.${CheckListRenderer.CSS.checkboxContainer}`
          );
      ...
      wrapperElement = make("ul", [
        CheckListRenderer.CSS.checklist,
        CheckListRenderer.CSS.itemChildren,
      ]) as HTMLUListElement;
    }
  ...
  public renderItem(content: string, meta: ChecklistItemMeta): HTMLLIElement {
    const itemWrapper = make("li", [
      CheckListRenderer.CSS.item,
      CheckListRenderer.CSS.item,
    ]);
  ...
  public getItemContent(item: Element): string {
    const contentNode = item.querySelector(
      `.${CheckListRenderer.CSS.itemContent}`
    );
  ...
  public getItemMeta(item: Element): ChecklistItemMeta {
    ...
    return {
      checked: checkbox
        ? checkbox.classList.contains(CheckListRenderer.CSS.itemChecked)
        : false,
    };
  }
  ...
  private toggleCheckbox(checkbox: Element): void {
    ...
    checkbox.addEventListener(
      "mouseleave",
      () => this.removeSpecialHoverBehavior(checkbox),
      { once: true }
    );
  }
