...
import type { API, PatternPasteEventDetail } from "@sse-editor/types";
...
export default class Embed {
  ...
  render(): HTMLElement {
    ...
    (template.content.firstChild as HTMLElement).setAttribute(
      "src",
      this.data.embed
    );
    (template.content.firstChild as HTMLElement).classList.add(
      this.CSS.content
    );
  ...
  onPaste(event: { detail: PatternPasteEventDetail }) {
    ...
    const embed = result
      ? embedUrl.replace(/<%= remote_id %>/g, id(result))
      : "";
  ...
  static prepare({ config = {} }: { config: EmbedConfig }) {
    ...
    const userServices = Object.entries(services)
      ...
      .filter(([key, service]) =>
        Embed.checkServiceConfig(service as ServiceConfig)
      )
      .map(([key, service]) => {
        const { regex, embedUrl, html, height, width, id } =
    ...
    Embed.services = entries.reduce<{ [key: string]: ServiceConfig }>(
      (result, [key, service]) => {
        if (!(key in result)) {
    ...
    Embed.patterns = entries.reduce<{ [key: string]: RegExp }>(
      (result, [key, item]) => {
  ...
  static checkServiceConfig(config: ServiceConfig): boolean {
    ...
    let isValid =
      Boolean(regex && regex instanceof RegExp) &&
