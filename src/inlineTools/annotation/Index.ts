 ...
 * Build styles
 */
import type { API, InlineToolConstructorOptions } from "@sse-editor/types";
import "./index.css";
import { IconAnnotation, IconClose } from "@sse-editor/icons";
...
interface AnnotationCSS {
  ...
  popupContent: string;
  popupInputTitle: string;
  popupInputText: string;
  popupFooter: string;
  popupSaveButton: string;
  popupRemoveButton: string;
}
...
export default class Annotation {
  ...
  static currentRange: Range | null = null;
  static currentElem: HTMLElement | null = null;
  ...
  getPopup() {
    ...
    if (!popup) {
      ...
      popup.classList.add(
        Annotation.CSS.popupClass,
        Annotation.CSS.popupHidden
      );
      ...
      popupCloseButton.innerHTML = IconClose;
      ...
      popupInputText.classList.add(
        this.api.styles.input,
        Annotation.CSS.popupInputText
      );
      ...
      popupButtonRemove.classList.add(
      ...
      popupButtonSave.classList.add(
        this.api.styles.button,
        Annotation.CSS.popupSaveButton
      );
  ...
  openPopup(title: string, text?: string): void {
    let popup = this.getPopup();

    let inputTitle = popup.querySelector(
      `.${Annotation.CSS.popupInputTitle}`
    ) as HTMLInputElement;
    ...
    let inputText = popup.querySelector(
      `.${Annotation.CSS.popupInputText}`
    ) as HTMLInputElement;
    ...
    let removeButton = popup.querySelector(
      `.${Annotation.CSS.popupRemoveButton}`
    ) as HTMLElement;
    if (removeButton) {
      if (Annotation.currentElem) {
  ...
  surround(range: Range): void {
    ...
    if (termWrapper) {
      ...
      this.openPopup(
        termWrapper.getAttribute("data-title") || "",
        termWrapper.getAttribute("data-text") || ""
      );
    } else {
   ...
   * @param {Range} range - selected fragment
   */
  wrap(range: Range, title: string, text: string): void {
    /**
  ...
  saveData(title: string, text: string): void {
    title = title.trim();
    text = text.trim();
